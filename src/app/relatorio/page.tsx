  ...
  const gerarPDFIndividual = async (relatorio: any) => {
    try {
      ...
      try {
        ...
        // Fotos - Movidas para a segunda página
        if (relatorio.fotos && relatorio.fotos.length > 0) {
          // Adicionar nova página
          doc.addPage();
          
          // Título de fotos na nova página
          doc.setFontSize(18);
          doc.setTextColor(40, 80, 200);
          doc.text('Fotos do Relatório', pageWidth / 2, margin, { align: 'center' });
          
          // Adicionar data do relatório
          doc.setFontSize(14);
          doc.setTextColor(0, 0, 0);
          doc.text(`Data: ${dataRelatorio}`, pageWidth / 2, margin + 10, { align: 'center' });
          
          // Linha separadora
          doc.setDrawColor(200, 200, 200);
          doc.line(margin, margin + 15, pageWidth - margin, margin + 15);
          
          yPos = margin + 25; // Reiniciar posição Y para a nova página
      ...
      <main className="w-full px-4 py-4">
        ...
        {activeTab === 'historico' && (
          <div className="bg-white shadow">
            <div className="p-4">
              <h3 className="text-lg font-medium text-gray-900 mb-4 -mt-1">Histórico de Relatórios</h3>
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                      <button
                        ...
                        className="h-full flex items-center text-gray-400 hover:text-gray-500 focus:outline-none transition-colors duration-200"
                        aria-label="Mostrar calendário"
                      >
                        <svg 
                          xmlns="http://www.w3.org/2000/svg" 
                          className={`h-5 w-5 ${mostrarCalendario ? 'text-primary-600' : ''}`} 
                          fill="none" 
                          viewBox="0 0 24 24" 
                          stroke="currentColor"
                        >
